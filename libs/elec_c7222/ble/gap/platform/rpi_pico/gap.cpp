...
BleError Gap::requestConnectionParameterUpdate(ConnectionHandle con_handle,
	...
	return map_btstack_status(
		gap_request_connection_parameter_update(con_handle,
												params.min_interval,
												params.max_interval,
	...
	switch (event_id) {
		...
		case EventId::LePeriodicAdvertisingSyncEstablished: {
			...
			const auto sync_handle =
				static_cast<ConnectionHandle>(hci_subevent_le_periodic_advertising_sync_establishment_get_sync_handle(
					event_data));
			for (const auto* handler : event_handlers_) {
		...
		case EventId::LePeriodicAdvertisingSyncLost: {
			const auto sync_handle =
				static_cast<ConnectionHandle>(hci_subevent_le_periodic_advertising_sync_lost_get_sync_handle(
					event_data));
			for (const auto* handler : event_handlers_) {
		...
		case EventId::LeAdvertisingSetTerminated: {
			...
			const auto con_handle =
				static_cast<ConnectionHandle>(hci_subevent_le_advertising_set_terminated_get_connection_handle(
					event_data));
			advertisement_enabled_ = false;
			for (const auto* handler : event_handlers_) {
				handler->onAdvertisingEnd(status, con_handle);
