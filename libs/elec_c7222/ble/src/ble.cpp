...
Ble* Ble::GetInstance(BtstackPort* btstack_port, bool enable_hci_logging) {
	if(instance_ == nullptr) {
		assert(btstack_port != nullptr && "First Ble::GetInstance call requires a non-null BtstackPort");
...
Ble* Ble::GetInstance(bool enable_hci_logging) {
	return GetInstance(nullptr, enable_hci_logging);
}
