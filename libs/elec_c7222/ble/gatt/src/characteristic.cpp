...
Characteristic::Characteristic(const Uuid& uuid, uint8_t properties,
	...
	declaration_attr_.SetValue(decl_value.data(), decl_value.size());
	...
	// Properties is a bitfield enum: each bit represents a capability (Read, Write, Notify, Indicate, etc.)
	...
	if ((static_cast<uint8_t>(properties) & static_cast<uint8_t>(Properties::kWriteWithoutResponse)) != 0)
	...
	uint8_t notify_indicate = (static_cast<uint8_t>(properties) & 
	...
	if ((static_cast<uint8_t>(properties) & static_cast<uint8_t>(Properties::kAuthenticatedSignedWrites)) != 0)
...
std::vector<Characteristic> Characteristic::ParseFromAttributes(const std::vector<Attribute>& attributes) {
	std::vector<Characteristic> characteristics;
	
	for (size_t i = 0; i < attributes.size(); ++i) {
		const Attribute& attr = attributes[i];
		
		// Look for Characteristic Declaration
		if (!Attribute::IsCharacteristicDeclaration(attr)) {
			continue;
		}
		
		// Parse the declaration value to extract properties, value handle, and UUID
		const uint8_t* decl_data = attr.GetValueData();
		size_t decl_size = attr.GetValueSize();
		
		if (decl_data == nullptr || decl_size < 5) {
			// Invalid declaration: need at least 1 byte properties + 2 bytes handle + 2 bytes UUID
			continue;
		}
		
		// Extract properties (1 byte)
		uint8_t props = decl_data[0];
		
		// Extract value handle (2 bytes, little-endian)
		uint16_t value_handle = static_cast<uint16_t>(decl_data[1]) | 
		                        (static_cast<uint16_t>(decl_data[2]) << 8);
		
		// Extract UUID (remaining bytes)
		Uuid char_uuid;
		if (decl_size == 5) {
			// 16-bit UUID (2 bytes)
		...
		uint16_t decl_handle = attr.GetHandle();
		characteristics.emplace_back(char_uuid, props, value_handle, decl_handle);
		Characteristic& characteristic = characteristics.back();
		
		// Find and attach the value attribute (should be next, but verify handle)
		if (i + 1 < attributes.size() && attributes[i + 1].GetHandle() == value_handle) {
			const Attribute& value_attr = attributes[i + 1];
			// Copy value data
			...
			// Copy callbacks if present
			if (value_attr.HasReadCallback()) {
				// Note: Can't copy std::function directly, would need redesign to support this
			}
			if (value_attr.HasWriteCallback()) {
				// Note: Can't copy std::function directly, would need redesign to support this
			}
			
			++i; // Skip the value attribute in next iteration
		}
		
		// Look for descriptors following the value attribute
		while (i + 1 < attributes.size()) {
			const Attribute& next_attr = attributes[i + 1];
			
			// Stop if we hit another characteristic declaration
			if (Attribute::IsCharacteristicDeclaration(next_attr)) {
				break;
			}
			...
			if (Attribute::IsClientCharacteristicConfiguration(next_attr)) {
				...
				// Add other descriptors (standard or custom)
				...
				// Not a descriptor, stop parsing this characteristic
				break;
			}
...
void Characteristic::SetReadCallback(Attribute::ReadCallback callback) {
	value_attr_.SetReadCallback(std::move(callback));
}

void Characteristic::SetWriteCallback(Attribute::WriteCallback callback) {
...
Attribute& Characteristic::EnableCCCD() {
	if (!cccd_) {
		...
		cccd_ = std::make_unique<Attribute>(
			Uuid(static_cast<uint16_t>(Attribute::AttributeType::kClientCharacteristicConfiguration)),
			static_cast<uint16_t>(Attribute::Properties::kRead | Attribute::Properties::kWrite | Attribute::Properties::kDynamic),
			initial_value,
...
Attribute& Characteristic::EnableSCCD() {
	if (!sccd_) {
		...
		sccd_ = std::make_unique<Attribute>(
			Uuid(static_cast<uint16_t>(Attribute::AttributeType::kServerCharacteristicConfiguration)),
			static_cast<uint16_t>(Attribute::Properties::kRead | Attribute::Properties::kWrite | Attribute::Properties::kDynamic),
			initial_value,
			sizeof(initial_value),
			0  // handle will be assigned later
...
Attribute& Characteristic::EnableExtendedProperties() {
	if (!extended_properties_) {
		...
		extended_properties_ = std::make_unique<Attribute>(
			Uuid(static_cast<uint16_t>(Attribute::AttributeType::kCharacteristicExtendedProperties)),
...
Attribute& Characteristic::SetExtendedPropertiesValue(ExtendedProperties config) {
	...
	extended_properties_->SetValue(ext_prop_bytes);
	return *extended_properties_;
}
...
Attribute& Characteristic::SetUserDescription(const std::string& description) {
	if (!user_description_) {
		...
		user_description_ = std::make_unique<Attribute>(
			Uuid(static_cast<uint16_t>(Attribute::AttributeType::kCharacteristicUserDescription)),
			static_cast<uint16_t>(Attribute::Properties::kRead),
		...
		user_description_->SetValue(desc_data);
	}
	return *user_description_;
}

Attribute& Characteristic::AddDescriptor(const Uuid& uuid, Attribute::Properties properties,
	const std::vector<uint8_t>& value, uint16_t handle) {
	// Check if this is a User Description descriptor
	if (uuid.Is16Bit() && uuid.Get16Bit() == static_cast<uint16_t>(Attribute::AttributeType::kCharacteristicUserDescription)) {
	...
	descriptors_.emplace_back(
		uuid,
		static_cast<uint16_t>(properties),
		value.data(),
		value.size(),
		handle
	);
...
std::ostream& operator<<(std::ostream& os, const Characteristic& characteristic) {
	...
	os << "  Declaration Handle: 0x" << std::hex << characteristic.GetDeclarationHandle() << std::dec << "\n";
	...
	if (cccd) {
		...
		if (cccd_data != nullptr && cccd->GetValueSize() >= 2) {
			...
			os << ", Value: 0x" << std::hex << std::setfill('0') << std::setw(4) 
			   << cccd_value << std::dec;
	...
	if (ext) {
		...
		if (ext_data != nullptr && ext->GetValueSize() >= 2) {
			...
			os << ", Value: 0x" << std::hex << std::setfill('0') << std::setw(4) 
			   << ext_value << std::dec;
			if (ext_value & static_cast<uint16_t>(Characteristic::ExtendedProperties::kReliableWrite)) {
				os << " (Reliable Write)";
			}
			if (ext_value & static_cast<uint16_t>(Characteristic::ExtendedProperties::kWritableAuxiliaries)) {
	...
	if (descriptor_count > 0) {
		...
		for (size_t i = 0; i < descriptor_count; ++i) {
			...
			if (desc) {
				os << "    [" << i << "] UUID: " << desc->GetUuid() 
				   << ", Handle: 0x" << std::hex << desc->GetHandle() << std::dec;
				const uint8_t* desc_data = desc->GetValueData();
...
uint16_t Characteristic::HandleAttributeRead(uint16_t attribute_handle, uint16_t offset, uint8_t* buffer, uint16_t buffer_size) {
	// Check declaration attribute
	if (declaration_attr_.GetHandle() == attribute_handle) {
		// Verify read permission
		if ((declaration_attr_.GetProperties() & static_cast<uint16_t>(Attribute::Properties::kRead)) == 0) {
			return static_cast<uint16_t>(BleError::kAttErrorReadNotPermitted);
		}
	...
	if (value_attr_.GetHandle() == attribute_handle) {
		// Verify read permission
		if ((value_attr_.GetProperties() & static_cast<uint16_t>(Attribute::Properties::kRead)) == 0) {
			return static_cast<uint16_t>(BleError::kAttErrorReadNotPermitted);
		}
	...
	if (extended_properties_ && extended_properties_->GetHandle() == attribute_handle) {
		// Verify read permission
		if ((extended_properties_->GetProperties() & static_cast<uint16_t>(Attribute::Properties::kRead)) == 0) {
			return static_cast<uint16_t>(BleError::kAttErrorReadNotPermitted);
		}
	...
	if (user_description_ && user_description_->GetHandle() == attribute_handle) {
		// Verify read permission
		if ((user_description_->GetProperties() & static_cast<uint16_t>(Attribute::Properties::kRead)) == 0) {
			return static_cast<uint16_t>(BleError::kAttErrorReadNotPermitted);
		}
	...
	for (auto& descriptor : descriptors_) {
		if (descriptor.GetHandle() == attribute_handle) {
			// Verify read permission
			if ((descriptor.GetProperties() & static_cast<uint16_t>(Attribute::Properties::kRead)) == 0) {
				return static_cast<uint16_t>(BleError::kAttErrorReadNotPermitted);
			}
...
BleError Characteristic::HandleAttributeWrite(uint16_t attribute_handle, uint16_t offset, const uint8_t* data, uint16_t size) {
	// Check value attribute
	if (value_attr_.GetHandle() == attribute_handle) {
...
Characteristic::SecurityLevel Characteristic::GetReadSecurityLevel() const {
	...
	uint8_t bit0 = (props & static_cast<uint16_t>(Attribute::Properties::kReadPermissionBit0)) ? 1 : 0;
	uint8_t bit1 = (props & static_cast<uint16_t>(Attribute::Properties::kReadPermissionBit1)) ? 2 : 0;
...
Characteristic::SecurityLevel Characteristic::GetWriteSecurityLevel() const {
	...
	uint8_t bit0 = (props & static_cast<uint16_t>(Attribute::Properties::kWritePermissionBit0)) ? 1 : 0;
	uint8_t bit1 = (props & static_cast<uint16_t>(Attribute::Properties::kWritePermissionBit1)) ? 2 : 0;
